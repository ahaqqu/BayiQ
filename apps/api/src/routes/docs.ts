...
export function registerDocRoutes(api: Hono<ApiEnv>): void {
  api.get(
    ...
    openAPIRouteHandler(api, {
      documentation: {
        info: { title: "BayiQ API", version: "1.0.0" },
      },
    }),
  );
  api.get("/docs", (c) =>
    c.html(
      `<!doctype html><html lang="en"><body><h1>BayiQ API</h1><a href="/openapi.json">OpenAPI</a> · schema ${SCHEMA_VERSION} · client ${CLIENT_VERSION}</body></html>`,
    ),
  );
}
